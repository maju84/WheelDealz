...
{
    public static void InitializeDB(WebApplication webApp)
    {
        /*
            This line is creating a new service scope. 
            In .NET Core, a service scope is used to control the lifetime of services retrieved 
            from the dependency injection container. 
            When the scope is disposed, any services that were retrieved from the scope 
            and implement IDisposable will also be disposed.
        */
    ...
    {
        // apply any pending migrations, create the database if it doesn't exist yet
        auctionDbContext.Database.Migrate(); 

        if (auctionDbContext.Auctions.Any())
        {
            // todo favor logging 
            Console.WriteLine("Auctions DB has already data seeded");    
            return;
        }


        var auctions = new List<Auction>
        {   // todo extract out into a json file and parse it
            // 1 Ford GT
            new Auction
            {
            ...
            new Auction
            {
                ...
                Status = Status.Live,
                Seller = "bob",
                EndsAt = DateTime.UtcNow.AddDays(19),
                Item = new Item
                {
                    Make = "Audi",
                    Model = "R8",
            ...
            new Auction
            {
                ...
                ReservePrice = 20000,
                Seller = "bob",
                EndsAt = DateTime.UtcNow.AddDays(48),
                Item = new Item
                {
                    Make = "Ford",
                    Model = "Model T",
